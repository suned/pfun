...
class TestEffect(MonadTest):
    ...
    def test_associativity_law(self, e, f, g, env):
        assert e.and_then(f).and_then(g).run(env) == e.and_then(
    ...
    def test_identity_law(self, value, env):
        assert effect.success(value).map(identity).run(env) == effect.success(
            value
        ).run(env)
    ...
    def test_left_identity_law(self, f, value, env):
        assert effect.success(value).and_then(f).run(env) == f(value).run(env)

    @given(anything(), anything())
    def test_right_identity_law(self, value, env):
        assert effect.success(value).and_then(effect.success).run(
            env
    ...
    def test_memoize(self):
        s = state.State(())
        e = (
            s.modify(lambda t: t + ("modify was called",))
            .discard_and_then(effect.success("result"))
            .memoize()
        )
    ...
    def test_repeat_error(self):
        s = schedule.recurs(2, schedule.spaced(timedelta(seconds=1)))
        assert effect.error("whoops").repeat(s).either().run(
            MockModules()
        ) == either.Left("whoops")

    def test_retry(self):
        s = schedule.recurs(2, schedule.spaced(timedelta(seconds=1)))
        assert effect.error("whoops").retry(s).either().run(
            MockModules()
    ...
    def test_race_repr(self):
        assert (
            repr(effect.success(0).race(effect.success(0)))
            == "success(0).race(success(0))"
        )

    def test_timeout_repr(self):
        assert (
            repr(effect.success(0).timeout(timedelta(seconds=1)))
...
class TestHTTP:
    ...
    def test_get_session(self):
        with mock.patch("pfun.http.aiohttp.ClientSession") as session:
...
class TestSQL:
    ...
    def test_fetch_one(self):
        with mock.patch("pfun.sql.asyncpg.connect") as connect_mock:
            connect_mock.return_value.close = CoroutineMock()
            connect_mock.return_value.fetchrow = CoroutineMock(
                return_value={"name": "bob", "age": 32}
            )
    ...
    def test_fetch(self):
        with mock.patch("pfun.sql.asyncpg.connect") as connect_mock:
            connect_mock.return_value.close = CoroutineMock()
            connect_mock.return_value.fetch = CoroutineMock(
                return_value=({"name": "bob", "age": 32},)
            )
    ...
    def test_as_type(self):
        ...
        assert sql.as_type(User)(results).run(None) == List((User("bob", 32),))


class TestClock:
    def test_sleep(self):
        with mock.patch("pfun.effect.asyncio.sleep") as sleep_mock:
