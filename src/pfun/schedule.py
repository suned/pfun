...
def exponential(delta: timedelta) -> Schedule[object]:
    ...
    return success(map(exp, itertools.count())).with_repr(f'exponential({repr(delta)})')
...
def recurs(n: int, schedule: Schedule[R]) -> Schedule[R]:
    ...
    """
    return schedule.map(lambda deltas: itertools.islice(deltas, n)).with_repr(f'recurs({n}, {repr(schedule)})')


@curry
def take_while(p: Callable[[timedelta], bool], schedule: Schedule[R]) -> Schedule[R]:
    """
...
def until(delta: timedelta, schedule: Schedule[R]) -> Schedule[R]:
    ...
    """
    return take_while(lambda d: d < delta, schedule).with_repr(f'until({repr(delta)}, {repr(schedule)})')
...
def jitter(schedule: Schedule[R]) -> Schedule[Intersection[R, HasRandom]]:
    ...
    """
    add_jitter = lift(lambda r, deltas: map(lambda d: timedelta(seconds=d.total_seconds() + r), deltas))
    return add_jitter(random(), schedule).with_repr(f'jitter({repr(schedule)})')
