...
We can achieve something similar in python using generators, which is what the `for_m` decorator
does for you:
...
```
<<<<<<< HEAD
The `Maybes[A, B]` class is just a type alias for `Generator[Maybe[A], A, B]`. As the name `for_m` implies this works
simply by iterating over the `Maybe` values generated by your function and sending the 'unwrapped' values back to your
generator using `and_then`.
=======
The `Maybes[A, B]` class is just a type alias for `Generator[Maybe[A], A, B]`.
>>>>>>> 5c232e331fc4a21eb98857cd82698f06d3efc7e0
